...
const Search = () => {
  ...
  // Busca detalhes consolidados do backend usando o novo microserviço de Comparação
  useEffect(() => {
    const idsValidos = colunas.filter(id => id !== "");
    if (idsValidos.length > 0) {
      const url = `${baseUrl}/requisicao?request=/comparar&ids=${idsValidos.join(",")}`;
      fetch(url)
        .then(res => res.json())
        .then(data => {
  ...
  // FunÃ§Ãµes de manipulaÃ§Ã£o das colunas
  ...
  const formataValor = (valor) => {
    if (typeof valor === "boolean") return valor ? "âœ” Sim" : "âœ˜ NÃ£o";
  ...
  // Calcula menor preÃ§o para destacar
          ...
          {categoriaSel && (
            <div>
              ...
              <div className="d-flex flex-wrap gap-3 align-items-end">
                    ...
                    >
                      ...
                      {listaProdutos.map((prod, i) => (
                        <option 
                          key={i} 
                          value={prod._id} 
                          // Desabilita se jÃ¡ estiver selecionado em OUTRA coluna
                ...
                {/* BotÃ£o de adicionar mais colunas */}
            ...
            <div className="table-responsive">
              <table className="table table-bordered text-center align-middle mb-0" style={{ tableLayout: "fixed", minWidth: `${200 + (colunas.length * 250)}px` }}>
                ...
                <tbody>
                  ...
                  <tr>
                    <td className="fw-bold text-start ps-4 bg-light">LanÃ§amento</td>
                    {colunas.map((id, index) => (
                  ...
                  {/* PREÃ‡O MÃ‰DIO */}
                  <tr>
                    <td className="fw-bold text-start ps-4 bg-light">PreÃ§o MÃ©dio</td>
                    {colunas.map((id, index) => {
                      const prod = detalhes[id];
